      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-4 relative">
              ...
              {timeline.map((step) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-5 rounded-lg border transition-all duration-300 flex items-center gap-5 group relative ${isActive
                        ? 'bg-[#004093] border-[#004093] text-white shadow-xl shadow-[#004093]/10 translate-x-2'
                        : 'bg-white border-slate-400 hover:border-[#004093]/30 text-slate-700 shadow-lg shadow-black hover:bg-white/90'
                      }`}
                  >
                    ...
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-linear-to-r from-[#004093] to-sky-900/10 opacity-100 rounded-2xl pointer-events-none z-0"
                      />
                    )}
                    ...
                    <div className="relative z-10 flex items-center justify-between w-full">
                      <div className="flex items-center gap-4">
                        ...
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 font-bold text-xs shrink-0 ${isActive
                            ? 'bg-[#FE9900] text-white shadow-md'
                            : 'bg-slate-100 text-slate-900 group-hover:bg-[#004093]/10 group-hover:text-[#004093]'
                          }`}>
                          {step.num}
                        </div>

                        <div>
                          <span className={`text-[10px] font-black uppercase tracking-wider block ${isActive ? 'text-slate-300' : 'text-slate-800'
                            }`}>
                            {step.time}
                          </span>
                      ...
                      <ArrowRight className={`w-4 h-4 transition-transform duration-300 ${isActive ? 'text-[#FE9900] translate-x-1' : 'text-slate-300 group-hover:text-[#004093]'
                        }`} />
                    </div>
                  </button>
                )
            ...
            {/* Bottom Help Notice */}
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-lg shadow-black/1 flex items-center gap-3">
          ...
          <div className="lg:col-span-7 flex">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white rounded-lg p-6 md:p-8 border border-slate-400/80 shadow-2xl flex flex-col justify-between w-full"
              >
                  ...
                  <div className="space-y-3 pt-6 border-t border-slate-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {activeData.checklist.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-2.5 p-3 bg-slate-50 rounded-lg border border-slate-400/40">
                ...
                {/* Footnote CTA */}
                <div className="pt-2 mt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <a
                    href="tel:1300672194"
                    className="bg-[#004093] hover:bg-[#FE9900] text-white text-[10px] font-black uppercase tracking-wider py-2.5 px-5 rounded-lg transition shadow-md w-full sm:w-auto text-center"
                  >
                    Call 1300 672 194
                  </a>
                </div>
