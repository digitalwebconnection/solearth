  ...
  return (
    <div className="bg-white mx-auto max-w-7xl py-0 md:py-20 px-4 sm:px-6 md:px-0" id="datasheet-section">
      ...
      <div className="overflow-hidden bg-slate-50/20 shadow-[0_15px_40px_rgba(10,31,68,0.03)] p-4 md:p-0">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 border-b border-slate-200 pb-6 mb-8">

          <button
            onClick={() => setActiveTab("specs")}
            className={`inline-flex items-center gap-2 px-5 sm:px-10 py-3 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-300 ${activeTab === "specs"
              ? "bg-[#1B74BB] text-white shadow-md shadow-[#1B74BB]/15 scale-[1.02]"
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-5 sm:px-10 py-3 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-300 ${activeTab === "models"
              ? "bg-[#1B74BB] text-white shadow-md shadow-[#1B74BB]/15 scale-[1.02]"
              : "bg-white border border-slate-300 text-slate-800 hover:border-slate-300"
              }`}
          >
          ...
          {hasPdfs && (
            <button
              ...
              className={`inline-flex items-center gap-2 px-5 sm:px-10 py-3 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-300 ${activeTab === "downloads"
                ? "bg-[#1B74BB] text-white shadow-md shadow-[#1B74BB]/15 scale-[1.02]"
                : "bg-white border border-slate-300 text-slate-800 hover:border-slate-300"
                }`}
            >
        ...
        <div className="relative min-h-[300px]">
          ...
          {activeTab === "specs" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 animate-fade-in text-left">
              {specs.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between py-4 border-b border-dashed border-slate-200 hover:bg-slate-50/30 px-2 sm:px-3 transition duration-150"
                >
                  <span className="text-xs md:text-sm font-extrabold text-[#1B74BB]">
                    {item.label}
                  </span>
                  <span className="text-xs md:text-sm font-semibold text-slate-850">
                    {item.value}
                  </span>
                </div>
              ))}
          ...
          {activeTab === "models" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in text-left">
              {modelsList.map((model, idx) => (
                <div
                  key={idx}
                  className="bg-white border border-slate-200 hover:border-[#1B74BB]/20 p-5 sm:p-6 rounded-2xl shadow-xl shadow-slate-100 hover:shadow-2xl transition duration-300 relative group flex flex-col justify-between min-h-[160px]"
                >
                  <div className="space-y-3">
                    <span className="inline-block text-[9px] font-black text-[#FCC200] bg-[#FCC200]/10 px-2 py-0.5 rounded uppercase tracking-wider">
                      {model.type} Option
                    </span>
                    <h5 className="text-base font-extrabold text-[#1B74BB] leading-tight">
                      {model.name}
                    </h5>
                  </div>

                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 text-xs font-semibold text-slate-900">
          ...
          {activeTab === "downloads" && hasPdfs && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in text-left">
              ...
              <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-[#1B74BB] to-[#00285c] p-5 sm:p-8 text-white flex flex-col justify-between min-h-[220px]">
                ...
                <div className="relative z-10 space-y-4">
                  <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center border border-white/10">
                    <Check size={18} className="text-[#FCC200]" />
                  </div>
                ...
                {product.productSection.pdfUrls?.map((item, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-[#1B74BB]/5 text-[#1B74BB] group-hover:bg-[#1B74BB] group-hover:text-white flex items-center justify-center transition-all duration-300 shrink-0">
                        <FileText size={20} />
                      </div>
                      <div>
                        ...
                        <span className="text-[9px] font-black text-slate-900 uppercase tracking-widest block mt-0.5">
                    ...
                    <a
                      ...
                      className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#FCC200] hover:bg-[#e08600] text-black px-5 py-3 rounded-xl text-xs font-black transition-all duration-300 shadow-sm shadow-[#FCC200]/15 hover:scale-[1.02] cursor-pointer"
                    >
                      <Download size={14} />
                      Download
                    </a>
