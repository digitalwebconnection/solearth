...
export default function AboutTimeline() {
  ...
  return (
    <section ref={containerRef} className="py-10 bg-slate-50 bg-[radial-gradient(#e2e8f0_1.5px,transparent_1.5px)] bg-size-[32px_32px] relative overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute top-1/4 left-1/4 w-180 h-180 rounded-full bg-[#28A8E0]/5 blur-[130px] pointer-events-none"
      />
      <motion.div
        animate={{
          scale: [1, 0.9, 1.1, 1],
        ...
        className="absolute bottom-1/4 right-1/4 w-180 h-180 rounded-full bg-[#F8C000]/4 blur-[130px] pointer-events-none"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-slate-200 -translate-x-1/2 pointer-events-none" />
          
          {/* Scroll-driven progress line */}
          <motion.div 
            style={{ scaleY, originY: 0 }}
            className="absolute left-4 md:left-1/2 top-0 bottom-0 w-[3px] bg-linear-to-b from-[#28A8E0] via-[#1D6FB8] to-[#F8C000] -translate-x-1/2 origin-top pointer-events-none"
          />
            ...
            return (
              ...
              >
                ...
                <motion.div 
                  ...
                  className="absolute left-4 md:left-1/2 top-8 md:top-12 -translate-x-1/2 -translate-y-1/2 z-20"
                >
                  <div className="relative flex items-center justify-center">
                    {/* Ring ping wave */}
                    <div className="absolute inset-0 w-8 h-8 rounded-full bg-[#1D6FB8]/20 animate-ping pointer-events-none" />
                    
                    <div className="w-8 h-8 rounded-full bg-white border-4 border-[#1D6FB8] flex items-center justify-center shadow-md group-hover:border-[#F8C000] group-hover:scale-110 transition-all duration-300">
                      <div className="w-2.5 h-2.5 rounded-full bg-[#0a1f44] group-hover:bg-[#F8C000] transition-colors duration-300" />
                    </div>
                  </div>
                </motion.div>

                {/* Content card side */}
                <div className="w-full md:w-[calc(50%-1.5rem)] pl-12 md:pl-0">
                  <motion.div
                    initial={{ opacity: 0, y: 35, x: isEven ? 30 : -30 }}
                    whileInView={{ opacity: 1, y: 0, x: 0 }}
                    viewport={{ once: true, margin: '-80px' }}
                    transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
                    className="relative bg-white border-2 border-slate-300 rounded-lg p-8 shadow-lg hover:shadow-xl hover:border-[#28A8E0]/30 hover:-translate-y-2 transition-all duration-500 overflow-hidden"
                  >
                    {/* Subtle glow reveal overlay */}
                    <div className="absolute -bottom-8 -right-8 w-28 h-28 rounded-full bg-slate-50 opacity-0 group-hover:opacity-100 group-hover:bg-[#28A8E0]/5 transition-all duration-500" />

                    {/* Giant Watermark Year text in the card background */}
                    <div className="absolute right-6 -bottom-6 select-none pointer-events-none text-7xl font-black text-slate-100/60 group-hover:text-[#28A8E0]/5 group-hover:scale-105 transition-all duration-500">
                      {event.year}
                    </div>

                    {/* Timeline card Header */}
                    <div className="flex items-center justify-between mb-6 relative z-10">
                      <div className="inline-flex px-4 py-1 text-sm font-black text-white bg-linear-to-r from-[#28A8E0] to-[#1D6FB8] rounded-full shadow-sm">
                        {event.year}
                      </div>
