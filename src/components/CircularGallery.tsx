...
class App {
  ...
  renderer!: Renderer;
  gl!: WebGL2RenderingContext | WebGLRenderingContext | any;
  camera!: Camera;
  scene!: Transform;
  planeGeometry!: Plane;
  ...
  onWheel(e: any) {
    ...
    const totalItems = this.mediasImages.length / 2;
    const isScrollingDown = deltaY > 0;
    const isScrollingUp = deltaY < 0;

    let shouldLock = false;
    if (isScrollingDown && this.targetIndex < totalItems - 1) {
      shouldLock = true;
    } else if (isScrollingUp && this.targetIndex > 0) {
      shouldLock = true;
    }
    ...
    if (shouldLock) {
      ...
      if (now - this.lastScrollTime > 300) {
        ...
        if (isScrollingDown) {
          this.targetIndex = Math.min(this.targetIndex + 1, totalItems - 1);
        } else {
        ...
        if (this.medias && this.medias[0]) {
          ...
          this.scroll.target = this.targetIndex * width;
        }

        if (this.onChange) {
          this.onChange(this.targetIndex);
        }
      }
    }
  }
  onCheck() {
    ...
    // Clamp to valid range so manual drag doesn't fly infinitely
    this.targetIndex = Math.max(0, Math.min(itemIndex, totalItems - 1));
    this.scroll.target = this.targetIndex * width;
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      appRef.current = new App(containerRef.current, {
        ...
        scrollEase,
        onChange,
        isScrollLinked: scrollProgress !== undefined
      });
    });
    return () => {
      isMounted = false;
  ...
  useEffect(() => {
    if (appRef.current && scrollProgress !== undefined) {
      ...
      if (app.medias && app.medias[0]) {
        const totalItems = app.mediasImages.length / 2;
        const width = app.medias[0].width;
        app.scroll.target = scrollProgress * (totalItems - 1) * width;
      }
    }
  }, [scrollProgress]);
